...
class SlowEER:
    @staticmethod
    def incrementalExpectedErrorReduction(sample, classes, X_train, y_train, base_model):
        eer = 0.0
        for cls in classes:
    ...
    @staticmethod
    def incrementalEERsampling(X_train, y_train, X_pool, y_pool, X_val, y_val, itr):
        ...
        for iteration in tqdm(range(itr)):
            ...
            for i, sample in enumerate(sampled):
                loss_reduction[i] = SlowEER.incrementalExpectedErrorReduction(sample, classes, X_train, y_train, model)
    ...
    @staticmethod
    def incrementalRandomsampling(X_train, y_train, X_pool, y_pool, X_val, y_val, itr):
        classes = np.unique(list(range(np.max(y_pool) + 1)))
        metrics = []
        for iteration in tqdm(range(itr)):
            logging.info(f"Train size is {X_train.shape}")
